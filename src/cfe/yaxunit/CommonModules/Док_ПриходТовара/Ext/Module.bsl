...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.ДобавитьТестовыйНабор(НСтр("ru = 'Документ Приход товара: Проверка проведения'"))
		...
		.ДобавитьТест("ПроверкаПроведенияНегатив", НСтр("ru = 'Проверка проведения: Пустая сумма'"), НСтр("ru = 'Негатив, ДЗ20, Левкина Ольга'")).УдалениеТестовыхДанных()
		;
...
Процедура ПроверкаПроведенияНегатив() Экспорт
	...
	Для Каждого ДокументКПроведению Из ДокументыКПроведению Цикл
		ДокументКПроведению.ПолучитьОбъект().ПроверитьЗаполнение();
		...
		ЮТест.ОжидаетЧто(Сообщения)
			...
			.Свойство("[0].Текст")
				.ИмеетТип("Строка")
				.Равно("Не заполнена колонка ""Сумма"" в строке 1 списка ""Товары""")
			;
